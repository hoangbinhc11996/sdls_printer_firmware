...
static void lcd_implementation_status_screen() {
  ...
  u8g.drawFrame(0, 0, LCD_PIXEL_WIDTH, LCD_PIXEL_HEIGHT);

  if (PAGE_CONTAINS(32, 32)) {
          // Percent complete
      u8g.drawLine(0, 32, LCD_PIXEL_WIDTH, 32);
  }

  if (PAGE_CONTAINS(10, 31)) {
          // Percent complete
      u8g.drawLine(100, 11, 100, 31);
    }

  //if (PAGE_CONTAINS(LCD_PIXEL_HEIGHT - STATUS_SCREENHEIGHT - 2, LCD_PIXEL_HEIGHT-1)) {
  ...
  //}
    ...
    // SD Card Symbol
    // 28 19 
    //if (card.isFileOpen() && PAGE_CONTAINS(16 - (TALL_FONT_CORRECTION), 25 - (TALL_FONT_CORRECTION))) {
    if (digitalRead(49) == LOW && PAGE_CONTAINS(16 - (TALL_FONT_CORRECTION), 25 - (TALL_FONT_CORRECTION))) {
      // Upper box
      u8g.drawBox(3, 16 - (TALL_FONT_CORRECTION), 8, 7);     // 42-48 (or 41-47)
      // Right edge
      u8g.drawBox(11, 18 - (TALL_FONT_CORRECTION), 2, 5);     // 44-48 (or 43-47)
      // Bottom hollow box
      u8g.drawFrame(3, 23 - (TALL_FONT_CORRECTION), 10, 4);  // 49-52 (or 48-51)
      // Corner pixel
      u8g.drawPixel(11, 17 - (TALL_FONT_CORRECTION));         // 43 (or 42)
    }
  #endif // SDSUPPORT
    ...
    // Progress bar frame
    //
    #define PROGRESS_BAR_X 15
    //#define PROGRESS_BAR_WIDTH (LCD_PIXEL_WIDTH - PROGRESS_BAR_X)
    #define PROGRESS_BAR_WIDTH 55

    if (PAGE_CONTAINS(24, 27 - (TALL_FONT_CORRECTION)))       // 49-52 (or 49-51)
      u8g.drawFrame(
        PROGRESS_BAR_X, 24,
    ...
    if (progress_bar_percent > 1) {
      ...
      if (PAGE_CONTAINS(25, 26 - (TALL_FONT_CORRECTION)))     // 50-51 (or just 50)
        u8g.drawBox(
          PROGRESS_BAR_X + 1, 25,
      ...
      //#if ENABLED(DOGM_SD_PERCENT)
        if (PAGE_CONTAINS(21, 29)) {
          // Percent complete
          u8g.setPrintPos(PROGRESS_BAR_WIDTH + 3 + PROGRESS_BAR_X, 29);
    ...
    if (PAGE_CONTAINS(12, 20)) {
      char buffer[10];
      duration_t elapsed = print_job_timer.duration();
      bool has_days = (elapsed.value >= 60*60*24L);
      uint8_t len = elapsed.toDigital(buffer, has_days);
      u8g.setPrintPos(SD_DURATION_X, 20);
      lcd_print(buffer);
    }
  ...
  if (PAGE_CONTAINS(XYZ_FRAME_TOP, XYZ_FRAME_TOP + XYZ_FRAME_HEIGHT - 1)) {
    u8g_uint_t delta = 11;
    #if ENABLED(XYZ_HOLLOW_FRAME)
      //u8g.drawFrame(0, XYZ_FRAME_TOP, LCD_PIXEL_WIDTH, XYZ_FRAME_HEIGHT); // 8: 29-40  7: 29-39 //original code
      u8g.drawFrame(0, XYZ_FRAME_TOP, LCD_PIXEL_WIDTH, XYZ_FRAME_HEIGHT); // 8: 29-40  7: 29-39 // edit by hoang chu 
      u8g.drawLine(1 * XYZ_SPACING + X_VALUE_POS - delta, 0, 1 * XYZ_SPACING + X_VALUE_POS - delta, INFO_FONT_HEIGHT + 1);
      u8g.drawLine(2 * XYZ_SPACING + X_VALUE_POS - delta, 0, 2 * XYZ_SPACING + X_VALUE_POS - delta, INFO_FONT_HEIGHT + 1);
      u8g.drawLine(2 * XYZ_SPACING + X_VALUE_POS - delta, 0, 2 * XYZ_SPACING + X_VALUE_POS - delta, INFO_FONT_HEIGHT + 1);
    #else
    ...
    if (PAGE_CONTAINS(XYZ_BASELINE - (INFO_FONT_HEIGHT - 1), XYZ_BASELINE)) {
      ...
      u8g.setPrintPos(0 * XYZ_SPACING + X_LABEL_POS + 5, XYZ_BASELINE - 1);
      lcd_print(':');
      u8g.setPrintPos(0 * XYZ_SPACING + X_VALUE_POS + 3, XYZ_BASELINE);
      _draw_axis_value(X_AXIS, xstring, blink);

      u8g.setPrintPos(1 * XYZ_SPACING + X_LABEL_POS, XYZ_BASELINE);
      lcd_printPGM(PSTR(MSG_Y)); 
      u8g.setPrintPos(1 * XYZ_SPACING + X_LABEL_POS + 5, XYZ_BASELINE - 1);
      lcd_print(':');
      u8g.setPrintPos(1 * XYZ_SPACING + X_VALUE_POS + 3, XYZ_BASELINE);
      _draw_axis_value(Y_AXIS, ystring, blink);

      u8g.setPrintPos(2 * XYZ_SPACING + X_LABEL_POS, XYZ_BASELINE);
      lcd_printPGM(PSTR(MSG_Z)); 
      u8g.setPrintPos(2 * XYZ_SPACING + X_LABEL_POS + 5, XYZ_BASELINE - 1);
      lcd_print(':');
  ...
  if (PAGE_CONTAINS(21 - INFO_FONT_HEIGHT, 30)) {
    lcd_setFont(FONT_MENU);
    u8g.setPrintPos(108, 21);
    lcd_print("FR");
    //lcd_print(LCD_STR_FEEDRATE[0]);

    lcd_setFont(FONT_STATUSMENU);
    u8g.setPrintPos(102, 30);
  ...
  if (PAGE_CONTAINS(50, 58))
  {
    u8g.setPrintPos(17, 58);
    u8g.print("MXm ");
  }
